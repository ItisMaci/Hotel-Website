...
if (isset($_GET['user_id']) && is_numeric($_GET['user_id'])) {
    ...
    if ($stmt->execute()) {
        header("Location: /web-project/index.php?page=administration"); // Redirect back to admin
    } else {
        echo "Error deleting record."; // Display error if query fails
    }
} else {
    header("Location: /web-project/index.php?page=administration"); // Redirect back if ID not valid
}
exit();
?>
